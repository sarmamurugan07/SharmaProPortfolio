...
// Click the hidden input when the icon is clicked
...
fileInput.addEventListener("change", () => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      // Set the global data for sending later
      userData.file.data = e.target.result.split(",")[1];
      userData.file.mime_type = file.type;
      
      // Visual feedback
      fileUploadWrapper.classList.add("file-uploaded");
      alert("Image attached! Note: WhatsApp links only support text. Please paste the text then attach the image manually in WhatsApp.");
    };
    reader.readAsDataURL(file);
  }
});

    const phoneNumber = "96878036914";

    // Toggle Chatbot UI Class
    chatbotToggler.addEventListener("click", () => {
        document.body.classList.toggle("show-chatbot");
        
        // If you want the toggle to automatically open the Bootstrap Modal:
    ...
    function sendWhatsAppMessage() {
        ...
        const descInput = document.getElementById('floatingTextarea');
    ...
    // Event Listeners
    document.getElementById('send-message').addEventListener('click', sendWhatsAppMessage);

    // Attach close logic to all possible close triggers
    const closeTriggerIds = ['close-message', 'btn-close'];
    ...
    // Optional: Keep your location map listener if needed
    const locationBtn = document.getElementById('location');
    if (locationBtn) {
        locationBtn.addEventListener('click', openGoogleMap);
    }
...
const handleOutgoingMessage = async (e) => {
    ...
    if (userData.file.data) {
        try {
            ...
            const blob = await (await fetch(`data:${userData.file.mime_type};base64,${userData.file.data}`)).blob();
            const file = new File([blob], "attachment.png", { type: userData.file.mime_type });
...
function openWhatsAppTextOnly() {
    const url = `https://wa.me/7639763307?text=${encodeURIComponent(userData.message)}`;
    window.open(url, '_blank');
}
